...
def getByName():
    name = request.form.get("name")
    return Response(BookService.getByName(name), mimetype='application/json')
...
def addBook():
    ...
    books = json.loads(BookService.getByName(name))
    isNew = 1
    if len(books) > 0:
        isNew = 0
        for b in books:
    ...
    BookService.insert(name, wordCount, isNew)
    return "添加成功"
